...
public class mGraphics
{
	...
	public void fillRectLight(int x, int y, int w, int h, int color, int alpha)
	{
		float alpha2 = 0.3f;
		setColor(color, alpha2);
		fillRect(x, y, w, h);
	...
	//mod fillrectbold
	public void fillRectBold(int x, int y, int w, int h, int color, int alpha)
	{
		float alpha2 = 2.5f;
		setColor(color, alpha2);
		fillRect(x, y, w, h);
	...
	public void drawLine(int x1, int y1, int x2, int y2)
	{
		...
		//mod		
        Graphics.DrawTexture(new Rect(vector.x - (float)num5, vector.y - (float)num4 - (float)num6, vector3.magnitude, 3f), texture2D);
        if (isClip)
		{
			GUI.EndGroup();
		}
