...
public class TField : IActionListener
{
	...
	public void paint(mGraphics g)
	{
		...
		//mod
		int n;
		bool isNumeric = int.TryParse(ChatTextField.gI().tfChat.getText(), out n);
		if (ChatTextField.gI().tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_ID_Map && isNumeric && n <= 155)
		{
			string nameMap = TileMap.mapNames[n];
			mFont.tahoma_7b_green2.drawString(g, "Tới: " + nameMap, this.x + this.width - 10
				, ChatTextField.gI().tfChat.y - ((!GameCanvas.isTouch) ? 12 : 17), 1);
		}

		paintInputTf(g, flag, x, y - 1, width, height + 5, TEXT_GAP_X + offsetX + x + 1, y + (height - mFont.tahoma_8b.getHeight()) / 2 + 2, Unikey._Unikey ? Unikey.Convert(paintedText) : paintedText, name);
