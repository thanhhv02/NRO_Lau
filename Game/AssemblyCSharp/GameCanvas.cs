...
public class GameCanvas : IActionListener
{
	...
	public void update()
	{
		#region mod update char speed, game time
		if (GameCanvas.gameTick % (40 * (int)Time.timeScale) == 0)
		{
			Time.timeScale = PickMob.speed;
			global::Char.myCharz().cspeed = PickMob.tdc * (int)Time.timeScale;
		}
		#endregion
		if (gameTick % 5 == 0)
		{
	...
	public static void paintBGGameScr(mGraphics g)
	{
		...
		int gH = GameScr.gH;
		g.translate(-g.getTranslateX(), -g.getTranslateY());
   //     if (PickMob.IsWallPaper)
   //     {
			//PickMob.FindFilesName();
			//g.drawImage(PickMob.wallPaper, GameCanvas.w / 2, GameCanvas.h / 2,3);
			//return;
   //     }
		g.setColor(9091064);
		g.fillRect(0, 0, w, h);
	...
	public void mapKeyPress(int keyCode)
	{
		...
		case 10:
			...
			if(Panel.isLock1 && !Panel.isAccept1 && Panel.isFriendLock1)
            {
					GameCanvas.endDlg();
	...
	public void paintChangeMap(mGraphics g)
	{
		...
		mFont.tahoma_7b_white.drawString(g, /*mResources.PLEASEWAIT*/ XmapController.mapnext + ((LoginScr.timeLogin <= 0) ? string.Empty : (" " + LoginScr.timeLogin + "s")), w / 2, h / 2, 2);
	}
	...
	{
		#region mod return "không thể đổi khu vực"	
