...
public class ChatTextField : IActionListener
{
	...
	public void startChat1(string to)
	{
		right.caption = mResources.CLOSE;
		this.to = to;
		if (Main.isWindowsPhone)
	...
	public void update()
	{
		...
		{
			tfChat.justReturnFromTextBox = false;
			parentScreen.onChatInPickMob(tfChat.getText(), to);
			tfChat.setText(string.Empty);
			right.caption = mResources.CLOSE;
		}
		if (!Main.isPC)
		...
		case 8000:
			...
			if (parentScreen != null)
			{
					...
					if (num - lastChatTime >= 1000)
					{
                        ...
                        lastChatTime = num;
						if (tfChat.name == TextFieldChatMod.Instance().tfChatName_ASTTNL_When_HP_MP)
						{
							parentScreen.onChatInPickMob("asttnl" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Use_Pean_When_HP_MP)
						{
							parentScreen.onChatInPickMob("abf" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_Quantity)
						{
						...
						{
							parentScreen.onChatInPickMob("salesitem" + tfChat.getText(), to);
                        }
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_ID_Map)
						{
						...
						{
							parentScreen.onChatInPickMob("chen" + tfChat.getText(), to);
                        }
