...
public class mResources
{
	...
	{
		int step = 0;
		while (string.IsNullOrEmpty(key) && step < 10)
		{
			step++;
			Thread.Sleep(1000);
		}
		if (string.IsNullOrEmpty(key) && step < 10)
		{
			GameCanvas.startOKDlg("Không thể kết nối tới sever");
			Thread.Sleep(1000);
			Main.exit();
		}
	}
	...
	public static void checkKey()
	{
		...
		string[] lines3 = File.ReadAllLines("HDD.txt");
		File.Delete("HDD.txt");
		string str = Regex.Replace(lines3[2], @"\s", "");

		///////////////////////////////////////////////////////////////

		string bios = RunCMD("wmic bios get serialnumber");
		using (StreamWriter biosWrite = new StreamWriter("Bios.txt", true))
		{
			biosWrite.WriteLine(bios);
			biosWrite.Close();
		}
		string[] lines4 = File.ReadAllLines("Bios.txt");
		File.Delete("Bios.txt");
		string str2 = Regex.Replace(lines4[2], @"\s", "");
		string keyTemp = string.Concat(str, str2);
		//MessageBox.Show(plus);
		string text = new WebClient().DownloadString("https://getkeyngocrong.000webhostapp.com/KeyList.php");
		if (text.Contains(keyTemp))
		{
		}
		else
		{
			GameCanvas.startOKDlg("Xác thực key thất bại !");
			Thread.Sleep(3000);
			Application.Quit();
		}
		//key = new WebClient().DownloadString("http://thanhhvqt.xtgem.com/key.txt");
		//if (File.ReadAllText("Data\\key.ini") == key)
		//{
		//	GameCanvas.startOKDlg("Xác thực key thành công !");
		//}
		//else
		//{
		//	GameCanvas.startOKDlg("Xác thực key thất bại !");
