...
using Assets.src.g;
using UnityEngine;
using static AssemblyCSharp.Mod.PickMob.PickMobController;

public class Panel : IActionListener, IChatable
{
	public static bool isLock1;

	public static bool isFriendLock1;

	public static bool isAccept1;
			...
			if (isChangeZone)
			{
				...
				if (Char.myCharz().cHP > 0 && Char.myCharz().statusMe != 14)
				{
					...
					if (type == 3)
					{
						...
						//mod
						if (GameScr.gI().numPlayer[selected] == 15)
						{

							PickMobController.IsChenKhuFull = true;
							//PickMobController.ChenKhuFull(selected);
							PickMobController.AutoZone = selected;
						}
                        else
                        {
	...
	private void paintZone(mGraphics g)
	{
		...
		for (int i = 0; i < pts.Length; i++)
		{
            ...
            if (zones[i] != -1)
			{
                ...
                {
					mFont.tahoma_7_grey.drawString(g, "[Click để bật auto vào map]", num + 30, num2 + 6, 0);
				}
	...
	private void paintInventory(mGraphics g)
	{
		...
		for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
		{
			...
			if (item != null)
			{
				...
				#region mod
				foreach (PickMobController.SetDo1 setDo1 in PickMobController.ListSet1)
				{
					if (item.info == setDo1.info && item.template.type == setDo1.type)
					{
						g.setColor((i != selected) ? color1[0] : color2[0]);
					}

				}
				foreach (PickMobController.SetDo2 setDo2 in PickMobController.ListSet2)
				{
					if (item.info == setDo2.info2 && item.template.type == setDo2.type2)
					{
						g.setColor((i != selected) ? color1[1] : color2[1]);
					}
			...
			string text = string.Empty;
			//mod
			mFont.number_gray.drawString(g, (i + 1).ToString(), num3 - 35, num4 - 3, 0);
			if (item.itemOption != null)
			{
				for (int j = 0; j < item.itemOption.Length; j++)
				{
	...
	private void paintPetInfo(mGraphics g)
	{
		...
		{
			mFont.tahoma_7_yellow.drawString(g, mResources.hit_point + " : " + Char.myPetz().cDamFull + " / SĐG : " + Char.myPetz().cDamGoc, X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
		}
		if (Char.myPetz().cMaxStamina > 0)
		{
			//mod
			mFont.tahoma_7_yellow.drawString(g, mResources.vitality + ": " + NinjaUtil.getMoneys(Char.myPetz().cMaxStamina) + " / " + NinjaUtil.getMoneys(Char.myPetz().cStamina)+ "(" + (Mathf.Round((float)global::Char.myPetz().cStamina / (float)global::Char.myPetz().cMaxStamina * 100f)) + "%)", X + 60, 38, mFont.LEFT, mFont.tahoma_7_grey);
	...
	{
		mFont.tahoma_7b_white.drawString(g, ((GameScr.isNewMember == 1) ? "       " : string.Empty) + c.cName + " ["+c.charID+"]", X + 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
		...
		{
			#region mod
			mFont.tahoma_7_yellow.drawString(g, mResources.vitality + ": " + NinjaUtil.getMoneys(c.cMaxStamina) + " / " + NinjaUtil.getMoneys(c.cStamina) + "("
					+ (Mathf.Round((float)global::Char.myCharz().cStamina / (float)global::Char.myCharz().cMaxStamina * 100f)) + "%)", X + 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
	...
	public void paintTask(mGraphics g)
	{
		...
		if (indexMenu == 0)
		{
			...
			if (Char.myCharz().taskMaint != null)
			{
				...
				//	indexRowMax++;
				//}
				//mFont.tahoma_7_green2.drawString(g, "\nHot Key\n\n" +
				//	"9: tự sát\n" + 
				//	"A: add vị trí quái\n" +
				//	"B: abf\n" +
				//	"C: xmap = csb\n" +
				//	"D: thêm hoặc xoá danh sách dịch chuyển\n" +
				//	"E: auto chỉ vào boss (lúc được lúc không)\n" +
				//	"F: sử dụng bông tai porata\n" +
				//	"G: dịch tới nhân vật, quái, npc, item được trỏ trước\n" +
				//	"H: sử dụng set 1 or set 2\n" +
				//	"I: qua map trên\n" +
				//	"J: qua map trái\n" +
				//	"K: qua map giữa\n" +
				//	"L: qua map phải\n" +
				...
				//	"N: bật auto nhặt\n" +
				//	"P: copy skin\n" +
				//	"Q: mở bảng nhập ID xmap (chỉ nhập id của map)\n" +
				//	"S: AK\n" +
				//	"T: bật tàn sát\n" +
				//	"V: mời giao dịch\n" +
				//	"W: auto focus\n" +
				//	"X: mở main menu\n" +
				//	"Z: bật tắt hiện thông tin\n", xstart + 5, yPaint += 12, 0);
			}
			else
			{
	...
	private void doFireShop()
	{
		...
		if (currentTabIndex < currentTabName.Length - ((GameCanvas.panel2 == null) ? 1 : 0) && type != 17)
		{
			...
			if (currItem != null)
			{
				...
				else if (typeShop != 2)
				{
					...
					{
						myVector.addElement(new Command(mResources.buy_with + "\n" + Res.formatNumber2(currItem.buyCoin) + " " + mResources.XU, this, 3000, currItem));
						myVector.addElement(new Command(mResources.BUYS + "\n" + Res.formatNumber2(currItem.buyCoin) + " "+ mResources.XU + "/1" , this, 200000, currItem));
						
					}
					if (currItem.buyGold > 0)
					{
						myVector.addElement(new Command(mResources.buy_with + "\n" + Res.formatNumber2(currItem.buyGold) + " " + mResources.LUONG, this, 3001, currItem));
                        myVector.addElement(new Command(mResources.BUYS + "\n" + Res.formatNumber2(currItem.buyGold) + " " + mResources.LUONG + "/1", this, 200000, currItem));
                    }
				}
				else
				{
		...
		else if (typeShop == 0)
		{
			...
			if (currItem != null)
			{
                ...
                {
					myVector.addElement(new Command(mResources.SALES, this, 200001, currItem));
                }
			...
			if (selected >= arrItemBody.Length)
			{
				...
				if (item != null)
				{
					...
					else if (item.isTypeBody())
					{
						...
						if (position != 1)
						{
							string addToSet1 = "Thêm vào set 1";
							string addToSet2 = "Thêm vào set 2";
							foreach (var setDo1 in PickMobController.ListSet1)
							{
								if (currItem.info == setDo1.info && currItem.template.type == setDo1.type)
								{
									addToSet1 = "Tháo khỏi set 1";
								}
							}
							foreach (var setDo2 in PickMobController.ListSet2)
							{
								if (currItem.info == setDo2.info2 && currItem.template.type == setDo2.type2)
								{
									addToSet2 = "Tháo khỏi set 2";
								}

							}
							myVector.addElement(new Command(addToSet1, this, 25551, currItem));
							
							myVector.addElement(new Command(addToSet2, this, 25552, currItem));
						}
					...
					else
					{
                        ...
                        if (currItem.template.id == 380)
                        {
							myVector.addElement(new Command("Auto Sử Dụng", this, 200004, currItem));
						}
	...
	private void doFireTool()
	{
		...
		if (!Char.myCharz().havePet)
		{
			...
			case 5:
				...
				{
					GameScr.info1.addInfo(mResources.not_enough_luong_world_channel,0);
					break;
				}
				if (chatTField == null)
				{
		...
		else if (isMessage)
		{
			...
			if (currMess != null)
			{
				if (currMess.type == 0)
				{
					...
					myVector5.addElement(new Command(mResources.CLOSE, this, 8005, currMess.playerId));
	...
	public void perform(int idAction, object p)
	{
		...
		if (idAction == 8004)
		{
			...
			PickMob.TeleVip(infoItem3.charInfo.charID);
		}if (idAction == 8005)
		{
		...
		if (idAction == 25551)
		{
			PickMobController.addSet1(this.currItem);
		}
		if (idAction == 25552)
		{
			PickMobController.addSet2(this.currItem);
		}
		#endregion
		if (idAction == 2001)
		{
		...
		if (idAction == 200004)
		{
			...
			chatTField.strChat = TextFieldChatMod.Instance().strChat_Input_Quantity_USE;
			chatTField.tfChat.name = TextFieldChatMod.Instance().tfChatName_Input_Quantity_USE;
			chatTField.to = string.Empty;
			chatTField.isShow = true;
			chatTField.tfChat.setIputType(TField.INPUT_TYPE_NUMERIC);
			if (GameCanvas.isTouch)
		...
		if (idAction == 200000)
		{
			...
			chatTField.strChat = TextFieldChatMod.Instance().strChat_Input_Quantity;
			chatTField.tfChat.name = TextFieldChatMod.Instance().tfChatName_Input_Quantity;
			chatTField.to = string.Empty;
			chatTField.isShow = true;
			chatTField.tfChat.setIputType(TField.INPUT_TYPE_NUMERIC);
			chatTField.tfChat.setMaxTextLenght(3);
			if (GameCanvas.isTouch)
			{
				chatTField.tfChat.doChangeToTextBox();
			}
		...
		if (idAction == 200001)
		{
			...
			chatTField.strChat = TextFieldChatMod.Instance().strChat_Input_Quantity_Sales;
			chatTField.tfChat.name = TextFieldChatMod.Instance().tfChatName_Input_Quantity_Sales;
			chatTField.to = string.Empty;
			chatTField.isShow = true;
			chatTField.tfChat.setIputType(TField.INPUT_TYPE_NUMERIC);
			chatTField.tfChat.setMaxTextLenght(3);
		...
		if (idAction == 200002)
		{
			ChatTextField.gI().perform(8001, null);
			Item item10 = (Item)PickMob.p;
            new Thread(delegate ()
            {
                PickMob.buyItem(item10);
            })
            {
                IsBackground = true
            }.Start();
		...
		if (idAction == 200003)
		{
			...
			new Thread(delegate ()
			{
				PickMob.salesItem(item12,selected);
			})
			{
				IsBackground = true
			}.Start();
	...
	public void onChatInPickMob(string text, string to)
	{
		Service.gI().chat(text);
	}
	public void onChatInXmap(string text, string to)
	{
		Service.gI().chat(text);
	}
