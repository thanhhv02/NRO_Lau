...
public class LoginScr : mScreen, IActionListener
{
	...
	public LoginScr()
	{
		...
		//mod mod by
		tfUser.setText(/*Rms.loadRMSString("acc")*/"Mod by Văn Thành");
	...
	public void setUserPass()
	{
		...
		{
			//mod
			tfUser.setText(/*text*/"Mod by Văn Thành");
		}
	...
	public void doLogin()
	{
        #region mod do login
        GameCanvas.connect();
        Service.gI().login(AutoLogin.Account, AutoLogin.Password, GameMidlet.VERSION, 0);
        if (Session_ME.connected)
        {
		...
		if (timeLogin > 0)
		{
			...
			if (currTimeLogin - lastTimeLogin >= 1000)
			{
				...
				if (timeLogin == 0)
				{
					...
					GameCanvas.instance.perform(9999, null);
					//GameCanvas.loginScr.doLogin();
				}
