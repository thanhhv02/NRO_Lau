...
namespace AssemblyCSharp.Mod.OnScreenMod
{
    ...
    public class OnScreenController
    {
        ...
        static int CharListWidth = GameCanvas.w - 130;
        static int CharListHeight = GameCanvas.h - 150;
        public static void UpdateKeyTouchControl()
        {
            if (OnScreen.nhanVat)
            {
                int numY = CharListHeight;
                int numX = CharListWidth;
                for (int a = 0; a < saveChar.Length; a++)
                {
                    if (saveChar[a] != null)
                    {
                        if (GameCanvas.isPointerHoldIn(numX, numY, 150, 12))
                        {
        ...
        public static void DSToChar(mGraphics g)
        {
            if (OnScreen.nhanVat)
            {
                ...
                int numY = CharListHeight;
                int numX = CharListWidth;
                int Distance_Between_Characters = 13;
                for (int j = 0; j < GameScr.vCharInMap.size(); j++)
                {
                    ...
                    {
                        g.fillRect(numX, numY, 150, 12, 65537, 100);
                        if(@char.cFlag > 0)
                        {
                            determineColor(@char, numX - 10, numY, g);
                        }
                        ...
                        saveChar[j] = @char;
                        numY += Distance_Between_Characters;
                    }
                    //"(",
            ...
            public static void ThongTinChucNang(mGraphics g)
            {
                if (OnScreen.IsThongTinCN&&(GameCanvas.gameTick / (int)Time.timeScale) % 100 != 0)
                {
                    mFont.tahoma_7b_white.drawString(g,
                        "Auto nhặt: " + (PickMob.PickMobAuto.IsAutoPickItems ? "On" : "Off")
                        + " - VĐH: " + (PickMob.PickMobAuto.IsVuotDiaHinh ? "On" : "Off")
                        + " - NSQ: " + (PickMob.PickMobAuto.IsNeSieuQuai ? "On" : "Off")
                        + " \nTàn sát: " + (PickMob.PickMobAuto.IsTanSat ? "On" : "Off")
                        + " - Update khu: " + (PickMob.PickMobAuto.ukhu ? "On" : "Off")
        ...
        public static void TimeTDHS(mGraphics g)
        {
            ...
            //        }
            //        flag = true;
            //        PickMob.PickMobAuto.speed = 1f;
            //        mFont.nameFontRed.drawString(g, @char.cName
            //        + "[" + NinjaUtil.getMoneys(@char.cHP) + "/"
            //        + NinjaUtil.getMoneys(@char.cHPFull) + "] - " + classHT
            //        , 220, 70, mFont.CENTER);
            //    }
            ...
            if (Char.myCharz().charFocus != null)
            {
                ...
                mFont.number_red.drawString(g, Char.myCharz().charFocus.cName
                    ...
                    , 220, 60, mFont.CENTER);
                if(Char.myCharz().charFocus.cTypePk == 5)
                {
                    mFont.nameFontRed.drawString(g, Char.myCharz().charFocus.cName
                    + "[" + NinjaUtil.getMoneys(Char.myCharz().charFocus.cHP) + "/"
                    + NinjaUtil.getMoneys(Char.myCharz().charFocus.cHPFull) + "] - " + classHT
                    , 220, 70, mFont.CENTER);
                }
        ...
        public static int quantityitem()
        {
            ...
            for(int i = 0; i < Char.myCharz().arrItemBag.Length; i++)
            {
                ...
                if(item !=null && item.template.id == OnScreen.idItem)
                {
                    q += item.quantity;
                }
            }
        ...
        {
            mFont.tahoma_7b_red.drawString(g, Nameitem()+": ("+ quantityitem()+")", GameCanvas.w - 200, GameCanvas.h - 230, mFont.CENTER, mFont.tahoma_7b_white);
        }
