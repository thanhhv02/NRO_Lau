...
public class GameCanvas : IActionListener
{
	...
	public void update()
	{
		...
		{
			Time.timeScale = PickMobAuto.speed;
			global::Char.myCharz().cspeed = PickMobAuto.tdc * (int)Time.timeScale;
		}
		#endregion
		if (gameTick % 5 == 0)
		{
	...
	public static void startOKDlg(string info)
	{
		...
		PickMobAuto.startOkDlg(info);
		if (info.ToLower().Contains("đã đạt số pha lê"))
		{
			PickMobAuto.DapDo2 = false;
		}
