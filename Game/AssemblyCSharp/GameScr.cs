...
using AssemblyCSharp.Mod.ConnectQLTK;
...
public class GameScr : mScreen, IChatable
{
	...
	public override void updateKey()
	{
		...
		else
		{
			...
			ConnectQLTK cn = new ConnectQLTK();
	...
	public bool isCharging()
	{
		...
		//	return true;
		//}
		#region mod can use skill when freez
		/*if (*/
	...
	public override void update()
	{
		...
		if (GameCanvas.gameTick % (3 * (int)Time.timeScale) == 0)
		{
			if (!Pk9rXmap.IsXmapRunning)
				PickMob.Update();
			PickMob.GameScrUpdate(vMob);
			Pk9rXmap.GameScrUpdate();
			SoundMn.gI().closeSound();
			//         if (LoadSetting.IsCompleteAutoGoback == false && GameCanvas.gameTick % (30*Time.timeScale)==0)
			//         {
			//	new Thread(delegate ()
			//	{
			...
			//	}.Start();
			//}
			//         if (LoadSetting.IsCompleteAutoMap == false && GameCanvas.gameTick % (30*Time.timeScale)==0)
			//         {
			...
			//	}.Start();
			//}
			//if (LoadSetting.IsCompleteAutoPean == false && GameCanvas.gameTick % (30*Time.timeScale)==0)
			//         {
			...
			//	}.Start();
			//}
			//         if (LoadSetting.IsCompleteAutoPet == false && GameCanvas.gameTick % (30*Time.timeScale)==0)
			//         {
			...
			//	}.Start();
			//}
			//         if (LoadSetting.IsCompleteAutoSkill == false && GameCanvas.gameTick % (30*Time.timeScale)==0)
			//         {
			...
			//	}.Start();
			//}
			//         if (LoadSetting.IsCompleteAutoTrain == false && GameCanvas.gameTick % (30*Time.timeScale)==0)
			//         {
	...
	{
		//mod xoa paint troi den  khirong xuat hien
		return;
		if (!GameCanvas.lowGraphic)
		{
	...
	public override void paint(mGraphics g)
	{
		...
		/////
		//if (OnScreen.IsXoaMap == false || OnScreen.IsXoaMapV == false)
  //      {
			if (isFreez || (isUseFreez && ChatPopup.currChatPopup == null))
			{
				...
				{
					//mod delete white screen when freez
					//g.setColor(16777215,0.2f);
					//g.fillRect(0, 0, GameCanvas.w, GameCanvas.h);
					if (dem <= 50)
					{
						//mod gamescr still working when freez
						//return;
					}
					if (isUseFreez)
					{
		...
		else
		{
			...
			else if (Char.myCharz().statusMe != 14)
			{
				if (gamePad.isSmallGamePad)
				{
					if (isAnalog != 1)
					{
						...
						mFont.number_gray.drawString(g, string.Empty + hpPotion, xHP + 20, yHP + 15, 2);
					}
					else if (isAnalog == 1)
					{
						g.drawImage((mScreen.keyTouch != 10) ? imgSkill : imgSkill2, xSkill + xHP - 1, yHP - 1, 0);
				...
				else if (!gamePad.isSmallGamePad)
				{
					if (isAnalog != 1)
					{
						...
						g.fillRect(xHP + 9, yHP + 10 + ((num3 != 0) ? (20 - num3) : 0), 22, (num3 == 0) ? 20 : num3);
						g.drawImage((mScreen.keyTouch != 10) ? imgHP1 : imgHP2, xHP, yHP, 0);
						mFont.nameFontRed.drawString(g, string.Empty + hpPotion, xHP + 20, yHP + 15, 2);
					}
					else
					{
						g.setColor(9670800);
						...
						mFont.tahoma_7b_white.drawString(g, string.Empty + hpPotion, xHP + 20, yHP + 15, 2);
					}
				}
			}
			if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num4; i++)
				{
                    ...
                    //mod display time TDHS
                    OnScreen.TimeCDSkill(skill, g, i);
                }
			}
			paintGamePad(g);
		}
	...
	public void openUIZone(Message message)
	{
		...
		//mod delete panel show when pet update
	...
	public void actionPerform(int idAction, object p)
	{
		...
		#region mod perform action
		PickMob.performMacSet(idAction);
		ShowMenu.Instance().ActionPerform(idAction,p);
		#endregion
		switch (idAction)
		{
		case 888351:
