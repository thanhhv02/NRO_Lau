...
public class Panel : IActionListener, IChatable
{
				...
				if (Char.myCharz().cHP > 0 && Char.myCharz().statusMe != 14)
				{
					...
					if (type == 3)
					{
						...
						if (GameScr.gI().numPlayer[selected] >= 15)
						{
							...
							PickMobAutoController.IsChenKhuFull = true;
							//PickMobController.ChenKhuFull(selected);
							PickMobAutoController.AutoZone = selected;
						}
						else
						{
							Service.gI().requestChangeZone(selected, -1);
					...
					{
						#region mod xmap -  capsule menu
						
						//Service.gI().requestMapSelect(selected);
						if (Panel.isMySelect)
						{
							ShowMenu.Instance().CapsuleSelect(this.selected);
							Panel.isMySelect = false;
                        }
                        else
                        {
	...
	private void paintMapTrans(mGraphics g)
	{
		...
		for (int i = 0; i < mapNames.Length; i++)
		{
			...
			if (num2 - cmy <= yScroll + hScroll && num2 - cmy >= yScroll - ITEM_HEIGHT)
			{
                ...
                if (isMySelect)
                {
					mFont.tahoma_7_red.drawString(g, mapNames[i], 5, num2 + 1, 0);
                }
                else
                {
	...
	private void paintInventory(mGraphics g)
	{
		...
		for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
		{
			...
			if (item != null)
			{
				...
				#region mod
					foreach (PickMobAutoController.SetDo1 setDo1 in PickMobAutoController.ListSet1)
					{
					...
					foreach (PickMobAutoController.SetDo2 setDo2 in PickMobAutoController.ListSet2)
					{
			...
			mFont.tahoma_7_green2.drawString(g, item.template.name + text + " ["+ item.template.id+"]", num3 + 5, num4 + 1, 0);
			string text2 = string.Empty;
			if (item.itemOption != null)
			{
				if (item.itemOption.Length > 0 && item.itemOption[0] != null && item.itemOption[0].optionTemplate.id != 102 && item.itemOption[0].optionTemplate.id != 107)
	...
	private void paintTab(mGraphics g)
	{
		...
		if (type == 14)
		{
            ...
            if (isMySelect)
            {
				g.setColor(13524492);
				g.fillRect(X + 1, 78, W - 2, 1);
				mFont.tahoma_7_red.drawString(g, "Menu Auto", startTabPos + TAB_W / 2, 59, mFont.CENTER);
				return;
			}
			g.setColor(13524492);
			g.fillRect(X + 1, 78, W - 2, 1);
	...
	private void paintBottomMoneyInfo(mGraphics g)
	{
		if (type != 13 || (currentTabIndex != 2 && !Equals(GameCanvas.panel2)))
		{
			...
			mFont.tahoma_7_yellow.drawString(g, NinjaUtil.getMoneys(Char.myCharz().luong) + string.Empty, X + 100, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
			g.drawImage(imgLuongKhoa, X + 140, H - 8, 3);
			mFont.tahoma_7_yellow.drawString(g, NinjaUtil.getMoneys(Char.myCharz().luongKhoa) + string.Empty, X + 150, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
		}
	}
				...
				if (item != null)
				{
					...
					else if (item.isTypeBody())
					{
						...
						if (position != 1)
						{
							...
							foreach (var setDo1 in PickMobAutoController.ListSet1)
							{
							...
							foreach (var setDo2 in PickMobAutoController.ListSet2)
							{
	...
	public void saleRequest(sbyte type, string info, short id)
	{
        ...
        if (PickMobAuto.isSell)
		{
			this.perform(3003, itemObject);
		}
	}
	...
	public void perform(int idAction, object p)
	{
		...
		if (idAction == 8004)
		{
			InfoItem infoItem3 = (InfoItem)p;
			//Service.gI().gotoPlayer(infoItem3.charInfo.charID);
			PickMobAuto.TeleVip(infoItem3.charInfo.charID);
		...
		if (idAction == 25551)
		{
			PickMobAutoController.addSet1(this.currItem);
		}
		if (idAction == 25552)
		{
			PickMobAutoController.addSet2(this.currItem);
		}
		#endregion
		if (idAction == 2001)
		{
		...
		if (idAction == 200002)
		{
			...
			Item item10 = (Item)PickMobAuto.p;
			new Thread(delegate ()
			{
				PickMobAuto.buyItem(item10);
			})
			{
				IsBackground = true
			}.Start();
		...
		if (idAction == 200003)
		{
			...
			new Thread(delegate ()
			{
				PickMobAuto.salesItem(item12, selected);
			})
			{
				IsBackground = true
			}.Start();
