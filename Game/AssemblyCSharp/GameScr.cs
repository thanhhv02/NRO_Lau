	...
	public override void update()
	{
		...
		if (GameCanvas.gameTick % (3 * (int)Time.timeScale) == 0)
		{
			if (!Pk9rXmap.IsXmapRunning)
				PickMobAuto.Update();
			PickMobAuto.GameScrUpdate(vMob);
			Pk9rXmap.GameScrUpdate();
			SoundMn.gI().closeSound();
            if (PickMobAuto.isLoadSetting == false)
            {

				ShowMenu.Instance().ActionPerform(230000, null);
				PickMobAuto.isLoadSetting = true;
				if(PickMobAuto.isLoadSetting == true)
                {
					GameScr.info1.addInfo("load setting",0);
