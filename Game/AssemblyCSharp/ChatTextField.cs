	...
	public void perform(int idAction, object p)
	{
		...
		case 8000:
			...
			if (parentScreen != null)
			{
				...
				{
					lastChatTime = num;
                        #region mod change chat

						if (strChat.Equals(TextFieldChatMod.Instance().strChat_ASTTNL_When_HP_MP))
						{
							parentScreen.onChatInPickMob("asttnl" + tfChat.getText(), to);
						}
						else if (strChat.Equals(TextFieldChatMod.Instance().strChat_Use_Pean_When_HP_MP))
						{
							parentScreen.onChatInPickMob("abf" + tfChat.getText(), to);
						}
						else if (strChat.Equals(TextFieldChatMod.Instance().strChat_Input_Quantity))
						{
							parentScreen.onChatInPickMob("buysitem" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_Quantity_Sales)
						{
							parentScreen.onChatInPickMob("sellitem" + tfChat.getText(), to);
						}
						else if (strChat.Equals(TextFieldChatMod.Instance().strChat_Input_ID_Map))
						{
							parentScreen.onChatInXmap("xmp" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_ID_ITEM_TO_PICK_LIST)
						{
							parentScreen.onChatInPickMob("addi" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_ID_ITEM_TO_PICK_BLOCK_LIST)
						{
							parentScreen.onChatInPickMob("blocki" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_SLN)
						{
							parentScreen.onChatInPickMob("sln" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_Auto_Vao_Khu)
						{
							parentScreen.onChatInPickMob("akhu" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_Quantity_USE)
						{
							parentScreen.onChatInPickMob("uitem" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_Auto_Chat)
						{
							parentScreen.onChatInPickMob("atc" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_Auto_ChatTG)
						{
							parentScreen.onChatInPickMob("atctg" + tfChat.getText(), to);
						}
						else if (tfChat.name == TextFieldChatMod.Instance().tfChatName_Input_Auto_ChatTG)
						{
							parentScreen.onChatInPickMob("atctg" + tfChat.getText(), to);
						}
						else if (strChat.Equals(TextFieldChatMod.Instance().strChat_Input_ID_TYPE_ITEM_PICK))
						{
							parentScreen.onChatInPickMob("addti" + tfChat.getText(), to);
						}
						else if (strChat.Equals(TextFieldChatMod.Instance().strChat_Input_ID_TYPE_ITEM_BLOCK))
						{
							parentScreen.onChatInPickMob("blockti" + tfChat.getText(), to);
						}
						else if (strChat.Equals(TextFieldChatMod.Instance().strChat_Input_ID_MOB))
						{
							parentScreen.onChatInPickMob("addm" + tfChat.getText(), to);
						}
						else if (strChat.Equals(TextFieldChatMod.Instance().strChat_Input_ID_TYPE_MOB))
						{
							parentScreen.onChatInPickMob("addtm" + tfChat.getText(), to);
						}
						else
						{
							parentScreen.onChatFromMe(tfChat.getText(), to);
